param(
    [ValidateSet('get_devtree','purchase_node','get_status','get_nearby_infrastructure','get_zone_catalog','get_tool_status','cancel_tool','get_outside_connections','batch_execute','build_network','build_road','cancel_batch','cancel_simulation_step','clear_zoning','demolish','diagnose_connections','execute_neighborhood','find_building_sites','get_batch','get_build_prefabs','get_buildings','get_camera','get_capabilities','get_city_diagnostics','get_city_management','get_city_map','get_city_state','get_neighborhood_plan','get_network','get_network_edges','get_operation','get_prefab_details','get_selected','get_services','get_simulation_step','get_tiles','get_water_facilities','get_zone_cells','inspect_entity','pause_for_analysis','ping','place_building','plan_neighborhood','preview_building','purchase_tiles','relocate_building','sample_terrain','save_checkpoint','set_camera','set_service_budget','set_simulation_speed','set_tax','simulate_step','trace_network','upgrade_network','zone_rectangle','stop')]
    [string]$Command = 'ping',
    [string]$ArgsJson = '{}',
